...
def register_function(app, collection, filter_values_collection):
    ...
    def get_filters(req: func.HttpRequest) -> func.HttpResponse:
        ...
        try:
            ...
            cached_filters = filter_values_collection.find_one({"_id": "current"})
            ...
            if cached_filters and "filters" in cached_filters:
                ...
                if last_updated:
                    logging.info(f"Returning filter values last updated at {last_updated}")
                ...
                logging.warning("No cached filter values found. Falling back to direct aggregation.")
