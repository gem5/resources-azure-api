...
def register_function(app, collection):
    ...
    @app.route(
        route="resources/get-dependent-workloads",
        ...
        try:
            ...
            if not resource_id:
                return create_error_response(
                    400, "Missing or invalid required parameter 'id'"
                )

            # Build pipeline to find dependent workloads
            pipeline = [
                {"$match": {"category": "workload"}},
                {
                    "$addFields": {
                        "resources": {"$objectToArray": "$resources"}
                    }
                },
                {"$unwind": "$resources"},
                {"$match": {"resources.v": resource_id}},
                {
                    "$group": {
                        "_id": "$id",
                    }
